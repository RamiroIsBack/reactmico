...
class AmigoDatos extends React.Component {
  ...
  handleGuardarCambios(event) {
    ...
    if (event.target.id === "datosEnvio") {
      ...
      let newDatosEnvio = {
        nombreCompletoEnvio: document.getElementById("nombreCompletoEnvio")
          .value,
      ...
      if (
        newDatosEnvio.nombreCompletoEnvio.length !== 0 &&
        newDatosEnvio.calle.length !== 0 &&
        newDatosEnvio.localidad.length !== 0 &&
        newDatosEnvio.provincia.length !== 0 &&
        newDatosEnvio.cp.length !== 0
      ) {
        newCurrentUser.datosEnvio = newDatosEnvio;
        this.props.cambiarCurrentUserModificables(
          "currentUserDatosEnvio",
          true
        );
      ...
      if (
        ...
        this.setState({ show: "no" }); //para q no muestre los expectations
        newCurrentUser.datosPersonales.nombre = document.getElementById(
          "nombre"
        ).value;
      ...
      if (this.state.mailExpectation1 && this.state.mailExpectation2) {
        this.setState({ show: "no" });
        newCurrentUser.datosPersonales.email = document.getElementById(
          "email"
        ).value;
        //al apretar el boton para cambiarlo, lo pongo a false
  ...
  handleCurrentUserModificables(event) {
    ...
    if (event.target.id === "datosEnvio") {
      if (this.props.currentUser) {
        if (this.props.currentUser.datosEnvio.cp) {
          //setting value of the inputs if there is somethihng in the DB
          this.setState({
            nombreCompletoEnvio: this.props.currentUser.datosEnvio
              .nombreCompletoEnvio,
      ...
      this.props.cambiarCurrentUserModificables(
        "currentUserDatosEnvio",
        !this.props.currentUserDatos.currentUserDatosEnvio
      );
    } else if (event.target.id === "foto") {
      this.props.cambiarCurrentUserModificables(
        "currentUserFoto",
        !this.props.currentUserDatos.currentUserFoto
      );
    } else if (event.target.id === "nombre") {
      if (this.props.currentUser) {
        if (this.props.currentUser.datosPersonales.nombre) {
          //setting value of the inputs if there is somethihng in the DB
          this.setState({
      ...
      if (this.props.currentUser.datosPersonales.email) {
        //setting value of the inputs if there is somethihng in the DB
        this.setState({
  ...
  renderDatosPersonales() {
    ...
    var loginIcon =
      "https://firebasestorage.googleapis.com/v0/b/micotextil-3f024.appspot.com/o/loginIcon.png?alt=media&token=9df1a1ea-8b37-482b-919e-b0fb3be6b273";
    if (this.props.currentUser) {
            ...
            <div className="col-xs-12 col-sm-9 col-md-9 col-lg-9">
              <label>
                <input
                  ...
                  onChange={this.handleGuardarCambios.bind(this)}
                  className="form-control"
                  id="foto"
                  placeholder="elige la foto de tus archivos"
                  style={{ display: "none" }}
                />
                <img
                  id="foto"
            ...
            {this.props.currentUserDatos.currentUserNombre && (
              <div>
                ...
                >
                  <button
                    id="nombre"
                    onClick={this.handleCurrentUserModificables.bind(this)}
                    style={{
                      border: "none",
                      borderRadius: 4,
            ...
            {!this.props.currentUserDatos.currentUserNombre && (
              <div>
                ...
                >
                  <button
                    id="nombre"
                    onClick={this.handleGuardarCambios.bind(this)}
                    style={{
                      border: "none",
                      borderRadius: 4,
                      backgroundColor: "black",
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td style={{ paddingTop: 0, paddingBottom: 0 }}>
                          <h6 style={estiloNom2}>
                            sin espacios ni tildes, solo numeros y letras{" "}
                          </h6>
                        </td>
                        <td style={{ paddingTop: 0, paddingBottom: 0 }}>
                          <h6
                  ...
                  <div>
                    <div className="hidden-xs col-sm-6 offset-sm-2 col-md-5 offset-md-3 col-lg-5 offset-lg-3">
                      <p style={{ marginBottom: 2 }}>
                        {" "}
                        {this.props.currentUser.datosPersonales.email}{" "}
                      </p>
                      {!this.props.currentUser.datosPersonales
                        .emailVerified && (
                        <div>
                          <div
                    ...
                    {!this.props.currentUser.datosPersonales.emailVerified &&
                      this.state.showInfoVerified && (
                        <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                          <p style={{ marginBottom: 2 }}>
                            porfavor verifica tu cuenta de correo haciendo click
                            en el enlace del email que te llegó de Mico
                          </p>
                          <p className="text-muted" style={{ marginBottom: 2 }}>
                            si no lo encuentras, probablemente esté en correo no
                            deseado{" "}
                          </p>
                          <p>
                            te lo volvemos a madnar pinchando{" "}
                            <a
                              style={{
  ...
  renderDatosParaEnvio() {
    if (this.props.currentUser) {
      if (this.props.currentUserDatos.currentUserDatosEnvio) {
        //mostrar los datos que hay
        return (
          <div>
            <div
              className="hidden-xs col-sm-6 offset-sm-2 col-md-5 offset-md-3 col-lg-5 offset-lg-3"
              style={{ paddingRight: 10 }}
            >
              <p style={{ marginBottom: 1 }}>
                {" "}
                {this.props.currentUser.datosEnvio.nombreCompletoEnvio}{" "}
              </p>
              <p style={{ marginBottom: 1 }}>
                {" "}
                {this.props.currentUser.datosEnvio.calle}{" "}
              </p>
              <p style={{ marginBottom: 1 }}>
                {" "}
                {this.props.currentUser.datosEnvio.localidad}{" "}
              </p>
              <p style={{ marginBottom: 1 }}>
                {" "}
                {this.props.currentUser.datosEnvio.cp +
                  " , " +
                  this.props.currentUser.datosEnvio.provincia}{" "}
              </p>
            </div>

            <div
              className="hidden-sm hidden-md hidden-lg visible-xs-block col-xs-9"
            ...
            <div
              className="col-xs-3 col-sm-4 col-md-4 col-lg-4"
              style={{ paddingRight: 0, paddingLeft: 0 }}
            >
              <button
                id="datosEnvio"
        ...
        //no hay datos en la BD o quiero modificarlos
        if (
          this.props.currentUser.datosEnvio.hayDatos ||
          this.state.quieroDatos
        ) {
          //quiere modificarlos
          return (
              ...
              <div className="form-group row">
                ...
                <div className="col-xs-12 col-sm-9 col-md-9 col-lg-9">
                  <input
                    ...
                    onChange={this.handleImputData.bind(this)}
                    className="form-control"
                    value={this.state.nombreCompletoEnvio}
                    id="nombreCompletoEnvio"
              ...
              <div className="form-group row">
                ...
                <div className="col-xs-12 col-sm-9 col-md-9 col-lg-9">
                  <input
                    ...
                    id="calle"
                    placeholder="calle del ejemplo 4"
                  />
                </div>
              </div>
              ...
              <div className="form-group row">
                ...
                >
                  <button
                    className="btn text-center form-control "
                    id="datosEnvio"
                    onClick={() => this.setState({ quieroDatos: false })}
                    style={{
                      paddingLeft: 5,
                      paddingRight: 0,
                      border: "1px solid",
          ...
          return (
            <div>
              <div
                className="col-xs-7 col-sm-8 col-md-8 col-lg-8"
                style={{ paddingRight: 2, paddingLeft: 0 }}
              >
                {this.props.lengua === "es"
                  ? "No necesitas rellenar datos de envio porque se te preguntará en el proceso de pago con PayPal."
                  : "non fai falla que enchas os datos do envio xa que se te pedirán no proceso do pago con PayPal."}
              </div>
              <div
                className="col-xs-5 col-sm-4 col-md-4 col-lg-4"
                style={{ paddingRight: 0, paddingLeft: 0 }}
              >
                <button
                  id="datosEnvio"
                  onClick={() => this.setState({ quieroDatos: true })}
                  style={{
                    border: "none",
                    borderRadius: 4,
                    backgroundColor: "black",
      ...
      <div>
        ...
        >
          ...
          <div className="col-xs-12 col-sm-10 col-md-10 col-lg-8">
            <div>{this.renderDatosPersonales()}</div>
          </div>
        </div>
        <div>
          <div
          ...
          >
            ...
            <div
              className="col-xs-12 col-sm-10 col-md-10 col-lg-8"
              style={{ paddingLeft: 0, paddingRight: 0 }}
            >
              <div>{this.renderDatosParaEnvio()}</div>
            </div>
          </div>
        </div>
      </div>
