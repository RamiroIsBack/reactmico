...
class Mapa extends Component {
  ...
  makeMarkers() {
    let markers = [];
    let mapsImg =
      "https://firebasestorage.googleapis.com/v0/b/micotextil-3f024.appspot.com/o/mapsPointer.jpeg?alt=media&token=4d2aee6c-56fb-466c-9285-0a49b5abc5fc";
    ...
    for (let i = 0; i < ferias.length; i++) {
      if (!ferias[i].caducada || ferias[i].mostrarCaducada) {
        ...
        if (this.props.lengua === "ga") {
          direccion = ferias[i].direccionGalego
            ? ferias[i].direccionGalego
            : ferias[i].direccion;
        }
        markers.push(
          <Marker
          ...
          >
            {ferias[i].showInfo && (
              <InfoWindow
                onCloseClick={() => this.props.handleClick(ferias[i].id)}
              >
                <div style={{ maxWidth: 200, padding: 0 }}>
                  ...
                  >
                    ...
                    >
                      <img
                        ...
                        onClick={() => this.handleClick(ferias[i].urlFeria)}
                      />
                    </a>
                  </div>
                  <div
                    className="container-fluid col-xs-7 col-sm-7 col-md-7 col-lg-7"
                    style={{ padding: 0 }}
                  >
                    <h5 style={{ marginTop: 2, marginBottom: 0 }}>
                      {ferias[i].nombre}
                    </h5>
                    <h6 style={{ marginTop: 2, marginBottom: 2 }}>
                      {ferias[i].fecha}
                    </h6>
                  </div>
                  <div
                    className="row btn"
                    onClick={() => this.handleClick(ferias[i].mapsLink)}
                    type="button"
                    id="mapsLink"
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ padding: 0 }}
                    >
                      <h6 id="mapsLink" style={{ whiteSpace: "initial" }}>
                        {direccion}{" "}
                      </h6>
                    </div>
                  </div>
