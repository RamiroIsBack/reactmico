...
export default (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case constants.CHANGE_QTTY:
      ...
      console.log(
        " from reducer PRODCUT_TO_CART: " +
          JSON.stringify(newState["cartList"][action.data.indice])
      );
      return newState;
    ...
    case constants.LOAD_CARRO:
      ...
      if (action.params === "okCarroReady") {
        //solo lo he subido a la DB
      } else if (action.params === "okCarro") {
        //he cogido de la DB as'i q hay q meterlo en cartList
        newState.cartList = action.data;
        newState.numProducts = action.data.length;

        newState["cartList"].forEach((value, index) => {
          if (!value.precioRebajado) {
            precioSubTotal += parseInt(value.precio, 10);
          } else {
            precioSubTotal += parseInt(value.precioRebajado, 10);
          }
        });
        newState["precioSubTotal"] = precioSubTotal;
      } else if (action.params === "okCarroVacio") {
        //se ha realizado una compra hay q vaciar el carro
        newState["cartList"] = [];
